...
const filing = (over = {}) => ({
  itemId: 'it-1', name: '未命名文件 (3).txt', course: '作業系統', modelCourse: '作業系統', kind: 'Notes',
  topic: '死結', confidence: 'high', evidence: '四個必要條件', seeded: false,
  toFolder: 'Courses/作業系統/Notes', learned: false, rejectedBefore: false, alsoKnownAs: '', ...over,
})
const rename = (over = {}) => ({
...
describe('learnedLines', () => {
  test('課名：模型說 A ・ 你要 B ・ 用過 N 次', () => {
    ...
    assert.equal(l.head, 'The model says “作業系統” · you say “OS”')
    assert.equal(l.why, 'used 2 times')
    assert.equal(l.id, 'p-1')
  })

  test('類型：講「這一堂課的」，不是全域的', () => {
    const l = learnedLines(learnedItem({ kind: 'file_kind', from: '作業系統／筆記', to: 'Lecture', times: 1 }))
    assert.equal(l.head, '作業系統／筆記 files · you call them “Lecture”')
    assert.equal(l.why, 'used 1 time')
  })

  test('退過貨：不可以寫成「你要（空白）」', () => {
    // 稽核 2026-09-20 之後「是哪一個建議」走 about（歸檔那一種才有；改名那一種是檔名，後端不回）
    const l = learnedLines(learnedItem({ kind: 'rejected', from: '', to: '', about: 'Courses/OS/Lecture' }))
    assert.equal(l.head, 'You turned down the suggestion “Courses/OS/Lecture”')
    assert.match(l.why, /not ticked by default/)
  })

  test('退過的是改名建議（後端不回檔名）→ 講得出有這回事，但不編一個名字出來', () => {
    const l = learnedLines(learnedItem({ kind: 'rejected', from: '', to: '', about: '' }))
    assert.equal(l.head, 'You turned down a rename suggestion')
    assert.match(l.why, /not ticked by default/)
  })
  ...
  test('times 壞掉（負數、NaN）就當成 1，不要印出奇怪的數字', () => {
    assert.equal(learnedLines(learnedItem({ times: -3 })).why, 'used 1 time')
    assert.equal(learnedLines(learnedItem({ times: 'x' })).why, 'used 1 time')
  })
})
...
describe('建議列上的兩個標記', () => {
  test('歸檔：learned 要講一句、rejectedBefore 要講一句', () => {
    ...
    assert.match(filingLines(filing({ learned: true })).why, /The location is the one you moved it to last time/)
    const back = filingLines(filing({ rejectedBefore: true }))
    assert.equal(back.rejected, true)
    assert.match(back.back, /You turned this suggestion down last time/)
    assert.match(back.back, /not ticked by default/)
  })

  test('歸檔：**「模型認為」那一句用模型自己的課名**', () => {
    const l = filingLines(filing({ course: 'OS', modelCourse: '作業系統', learned: true, toFolder: 'Courses/OS/Notes' }))
    assert.match(l.why, /The model thinks: 作業系統 \/ 死結/)
    assert.equal(l.head, '未命名文件 (3).txt → Courses/OS/Notes')
  })

  test('歸檔：舊資料夾還在的時候要講一句（預期行為 12）', () => {
    const l = filingLines(filing({ course: 'OS', learned: true, alsoKnownAs: '作業系統', toFolder: 'Courses/OS/Notes' }))
    assert.match(l.note, /You used to call it “作業系統”; that folder is still there/)
    assert.ok(!filingLines(filing()).note.includes('你之前把它叫'))
  })

  test('改名：learned 與 rejectedBefore 同樣看得到', () => {
    assert.match(renameLines(rename({ learned: true })).why, /The course part is spelled the way you changed it last time/)
  ...
  test('舊的後端沒有這幾個欄位 → 當成沒有（面板不可以因此壞掉）', () => {
    const old = { itemId: 'x', name: 'a.txt', course: '作業系統', kind: 'Notes', toFolder: 'Courses/作業系統/Notes' }
    const l = filingLines(old)
    assert.equal(l.rejected, false)
    assert.equal(l.back, '')
    assert.match(l.why, /The model thinks: 作業系統/)
  })
})
...
describe('面板的「它學到的事」', () => {
  ...
  test('學過之後列出來，每一列一個「忘掉」', async t => {
    const ui = await open(t, { learned: [learnedItem(), learnedItem({ id: 'p-2', kind: 'file_kind', from: '作業系統／筆記', to: 'Lecture' })] })
    ...
    assert.equal(buttons[0].textContent, 'Forget')
    assert.match(ui.$('cleanup-learned').textContent, /The model says “作業系統” · you say “OS”/)
    assert.match(ui.$('cleanup-learned').textContent, /never moves a file on its own/)
  })

  test('按「忘掉」→ DELETE /learned { ids: [那一條] }，而且重讀建議那兩區', async t => {
    const ui = await open(t, {
      learned: [learnedItem()],
      filings: [filing({ learned: true, course: 'OS', toFolder: 'Courses/OS/Notes' })],
    })
    ...
    assert.ok(after.some(c => c.path === '/file/suggestions'), 'Forget之後建議要重讀')
    assert.ok(after.some(c => c.path === '/rename/suggestions'), 'Forget之後建議要重讀')
    assert.match(ui.$('cleanup-result').textContent, /Forgotten/)
  })

  test('**沒有「全部忘掉」這種按鈕**：面板一次只忘一條', async t => {
    const ui = await open(t, { learned: [learnedItem(), learnedItem({ id: 'p-2' })] })
    for (const b of ui.$('cleanup-learned').all('BUTTON')) assert.equal(b.textContent, 'Forget')
  })

  test('丟掉過幾條要講出來（不是安靜地消失）', async t => {
    const ui = await open(t, { learned: [learnedItem()], evicted: 7 })
    assert.match(ui.$('cleanup-learned').textContent, /the 7 oldest and least-used entrys got dropped/)
  })
  ...
  test('**畫面上一個絕對路徑都沒有**（預期行為 11）', async t => {
    const ui = await open(t, {
      learned: [learnedItem(), learnedItem({ id: 'p-2', kind: 'rejected', from: 'Courses/OS/Lecture', to: '' })],
    })
...
describe('退過貨的建議：列出來，但不預設勾（預期行為 5）', () => {
  test('歸檔區：勾選框沒有勾，而且畫面上講了原因', async t => {
    ...
    assert.match(ui.$('cleanup-filings').textContent, /You turned this suggestion down last time, so it is not ticked by default/)
  })

  test('改名區：一樣', async t => {
    const ui = await open(t, { renames: [rename({ rejectedBefore: true })] })
    assert.equal(ui.$('cleanup-renames').all('INPUT')[0].checked, false)
    assert.match(ui.$('cleanup-renames').textContent, /You turned this suggestion down last time/)
  })

  test('沒退過的那幾列不會多出那一句', async t => {
    const ui = await open(t, { filings: [filing()] })
    assert.equal(ui.$('cleanup-filings').all('INPUT')[0].checked, false)
    assert.ok(!ui.$('cleanup-filings').textContent.includes('You turned this suggestion down'))
  })
...
describe('createLearned（單獨測，不掛 DOM）', () => {
  ...
  test('忘掉一條不在清單上的 → 不送請求', async () => {
    ...
    await store.load()
    await assert.rejects(() => store.forget('不存在'), /no longer on the list/)

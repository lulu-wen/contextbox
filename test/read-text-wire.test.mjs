...
describe('6 ・ 沒有文字層不算錯', () => {
  test('掃描版 PDF：has_text 0、text 是空的、沒有 reason', t => {
    ...
    assert.equal(s.problems.filter(m => m.includes('could not be made sense of')).length, 0)
  })
...
describe('10 ・ worker 連續死 MAX_WORKER_DEATHS 次就放棄這一輪', () => {
  test('全部都卡住：前 3 個記逾時，剩下的留到下一輪，掃描照常完成、記一條 problem', t => {
    ...
    const gaveUp = s.problems.filter(m => m.includes('failed 3 times in a row'))
  ...
  test('中間讀成功一次，連續就歸零（不是累計）', t => {
    ...
    assert.equal(r.textsPending, 0)
    assert.equal(s.problems.filter(m => m.includes('failed 3 times in a row')).length, 0)

...
Ten images, referenced from `docs/UserGuide.md` as `![…](images/Ui.png)`. The guide reads fine without
...
| `Sections.png` | The six section tabs inside the cleanup panel — Cleanup / Bursts / Suggested names / Filing / Learned / Settings — with their counts, at least one of them showing 0 so the greyed-out state is visible. Settings has no count, which is the point of showing it here | `cleanup scan` and `rename`/`file` listed at least once, so several sections have rows. Do not press D — demo mode hides this strip |
...
| `Settings.png` | The Settings section: the read-only switch, the three model boxes and the screenshots switch, with the "Takes effect the moment you save" and "Needs the pet restarted" lines at the bottom. **The key environment variable box must show a name** (`CONTEXTBOX_MODEL_KEY`) — if anything that could be a key is on screen, do not take the shot | Open the panel and go to Settings. The sandbox config from `demo-setup.mjs` is already filled in, so nothing on screen is your real endpoint |

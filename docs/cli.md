...
- 「原檔都還在原位」**只在每一個沒成功的都是 `failed` 時才說**。有 `unknown` 的話不可以這樣說 ——
  ...
  再套用一次這份計畫會把它接完。」（「接完」只對還是 `proposed` 的計畫成立；`partial`／`error` 的計畫再套用是原樣回傳，
...
- **計畫是一次性的**（第二輪 R2-3）：`partial`／`error` 的計畫再 `apply <id>` 會原樣回傳（印同樣的逐項結果、同樣的離開碼），
  **不會重試**失敗的那幾個 —— 使用者可能剛把其中的檔放回來，遲到的重送不可以再把它搬走。
  要重試失敗的檔：`cleanup scan` 重掃，再 `cleanup apply` 建一份新計畫（失敗的檔還是候選）。
  做到一半中斷的 `proposed` 計畫（見「卡住的計畫」）`apply <id>` 會接著做完。

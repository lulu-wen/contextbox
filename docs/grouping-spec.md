...
**狀態：規格，實作中。** 純邏輯與測試已經在 `core/grouping.ts`、`test/grouping.test.mjs`
（commit fb23b6f）；逐檔 schema 的合併與 CLI 還沒做。

這份規格改過兩次，兩次都是**實機資料推翻了紙上的假設**。最後一節記了改的過程 ——
那三個假設錯得很典型，值得留著。
...
12. `node cli.mjs group` —— 產生對照表並印出來（不動檔案）
13. `node cli.mjs group --apply [分類名…]` —— 套用；不給就是全部
14. 面板多一區「提議的分法」，跟改名／歸檔同一種列：勾選框、理由、View contents、
    一組一顆按鈕，旁邊永遠有「復原」。空的時候整區不顯示。

---

## 測試要先紅的那幾條

已經綠的在 `test/grouping.test.mjs`（27 條，commit fb23b6f）：
...
7. 分類那一次的 payload **不含檔名、不含 evidence、不含原文**（逐字比對）
8. `group` 執行完，磁碟上**一個檔都沒動**
9. `course` 說得出來的檔不會同時出現在兩邊的建議裡
10. 重新分類一次 → 舊對照表整份被取代

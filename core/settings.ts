...
function secretsOf(raw: unknown): string[] {
  ...
  return [m.baseUrl, m.keyEnv].filter((v): v is string => typeof v === 'string' && v.length >= SECRET_MIN)
}
...
/** 少了某一條的複本（不動原本那一份 —— 外面那個迴圈還在用它）。 */
function omit(leaves: Map<string, unknown>, drop: string): Map<string, unknown> {
 ...
 *   2. 把這一欄（只有這一欄）退回原本的值再 normalize 一次，意見會少一句 ——
 *      少掉的那幾句就是這一欄害的。
 *
 * 為什麼要兩個都看：
...
export function applyPatch(path: string, patch: unknown, env: Env = process.env,
  ...
  if (broken) {
    ...
    return { ok: false, code: 'WRITE_FAILED', error: `${broken} Nothing was changed. Fix that file, or move it aside, first.` }
  }
  ...
  for (const [p, asked] of leaves) {
    ...
    const without = normalize(mergeInto(base, omit(leaves, p)))
    const blame = extraProblems(full.problems, without.problems)
  ...
  return {
    ...
    restartNeeded: [...leaves.keys()].filter(p => RESTART.includes(p)),
    config: full.config,
  }
}

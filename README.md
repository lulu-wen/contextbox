...
打開它印的網址（`http://127.0.0.1:7391/?k=…`），把基本資料填一填。鑰匙已經幫你帶好了。
...
檢查完到真的讀檔之間會再確認一次是同一個檔案。**只搬不刪** —— 清理只會把檔搬進隔離區，
七天內都放得回來。唯一會刪檔的地方是「清空隔離區」，要你確認兩次；整個專案只有那一處刪檔的呼叫，
而且有測試在守這條規則。
